...
ENT.JumpSound = Sound("npc_tara/jump.mp3")
ENT.JumpHighSound = Sound("npc_tara/spring.mp3")
ENT.TauntSounds = {
	Sound("npc_tara/pieceofcake.mp3"),
}
local chaseMusic = Sound("npc_tara/panic.mp3")
...
local npc_tara_acquire_distance =
	CreateConVar("npc_tara_acquire_distance", 2300, FCVAR_NONE,
	"The maximum distance at which tara will chase a target.")

local npc_tara_spawn_protect =
	CreateConVar("npc_tara_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, tara will not target players or hide within 200 units of \z
	a spawn point.")

local npc_tara_attack_distance =
	CreateConVar("npc_tara_attack_distance", 80, FCVAR_NONE,
	"The reach of tara's attack.")

local npc_tara_attack_interval =
	CreateConVar("npc_tara_attack_interval", 0.2, FCVAR_NONE,
	"The delay between tara's attacks.")

local npc_tara_attack_force =
	CreateConVar("npc_tara_attack_force", 700, FCVAR_NONE,
	"The physical force of tara's attack. Higher values throw things \z
	farther.")

local npc_tara_smash_props =
	CreateConVar("npc_tara_smash_props", 1, FCVAR_NONE,
	"If set to 1, tara will punch through any props placed in their way.")

local npc_tara_allow_jump =
	CreateConVar("npc_tara_allow_jump", 1, FCVAR_NONE,
	"If set to 1, tara will be able to jump.")

local npc_tara_hiding_scan_interval =
	CreateConVar("npc_tara_hiding_scan_interval", 3, FCVAR_NONE,
	"tara will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant taras are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_tara_hiding_repath_interval =
	CreateConVar("npc_tara_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to tara's hiding spot will be redetermined every X seconds.")

local npc_tara_chase_repath_interval =
	CreateConVar("npc_tara_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of tara's target will be redetermined every \z
	X seconds.")

local npc_tara_expensive_scan_interval =
	CreateConVar("npc_tara_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_tara_force_download =
	CreateConVar("npc_tara_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download tara resources \z
...
if npc_tara_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("tara_nag")
util.AddNetworkString("tara_navgen")
...
local VECTOR_tara_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_tara_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_tara: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other taras, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "taraMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_tara" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why tara isn't working.
	net.Start("tara_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_tara: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_tara: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_tara: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_tara: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_tara: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, tara in pairs(ents.FindByClass("npc_tara")) do
		tara:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "taraNavGen", navEndGenerate)
	else
		print("npc_tara: nav_generate failed to initialize")
...
concommand.Add("npc_tara_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_tara: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. tara doesn't use
	...
	elseif isConsole then
		print("npc_tara: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_tara_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_tara_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_tara_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_tara_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. tara means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_tara_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_tara_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_tara_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_tara_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_tara_attack_distance:GetInt()
		...
		local repathInterval = npc_tara_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_tara_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_tara_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_tara_hiding_repath_interval:GetFloat()
...
local MAT_tara = Material("npc_tara/tara")
killicon.Add("npc_tara", "npc_tara/killicon", color_white)
language.Add("npc_tara", "tara ")
...
local lastPanic = 0 -- The last time we were in music range of a tara.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_tara_music_volume =
	CreateConVar("npc_tara_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by tara. (0-1, where 0 is muted)")

-- If another tara comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, taras do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_tara_PANIC_COUNT taras are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of tara.
local MUSIC_tara_PANIC_COUNT = 8

local MUSIC_tara_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_tara_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_tara") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("taraPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_tara_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_tara" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_tara_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear tara taunt us.
...
local function startTimer()
	if not timer.Exists("taraPanicMusicUpdate") then
		timer.Create("taraPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_tara)

	-- Get the normal vector from tara to the player's eyes, and then compute
	...
	-- tara should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("taraHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("taraHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("tara is studying this map.", "taraHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "taraHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "taraHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "taraHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "taraHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("tara_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "taraNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. tara doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_tara_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for tara to figure \z
		this map out.\n\z
...
net.Receive("tara_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! tara doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet tara-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! tara doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to tara.\n\z
			\n\z
			If you ARE the server host, you can run npc_tara_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not tara-compatible!",
			"Ok", nil,
